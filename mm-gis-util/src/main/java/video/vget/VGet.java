...
import video.vget.info.VGetParser;
import video.vget.info.VideoFileInfo;
import video.vget.info.VideoInfo;
import video.vget.info.VideoInfo.States;
import video.vget.vhs.VimeoParser;
import video.vget.vhs.YouTubeParser;
    ...
    void retry(VGetParser user, AtomicBoolean stop, Runnable notify, Throwable e) {
        ...
        while (!retracted) {
            for (int i = RetryWrap.RETRY_DELAY; i >= 0; i--) {
                ...
                if(o == -10){
                    throw new DownloadInterruptedError("No exists mostly");
                }
                try {
                    Thread.sleep(1000);
